...
public class ChoreoAllianceFlipUtil {
    ...
    public static Rotation3d flip(Rotation3d rotation) {
        return new Rotation3d(rotation.getX(), rotation.getY(), flip(rotation.toRotation2d()).getRadians());
    }
    ...
    public static Supplier<Optional<Pose2d>> optionalFlippedPose2d(
        ...
        return () ->
            doFlip
                ? allianceOpt.get().flatMap(ally -> poseOpt.map(pose -> ally == Alliance.Red ? flip(pose) : pose))
                : poseOpt;
    }
        ...
        return () ->
            doFlip
                ? allianceOpt
                    .get()
                    .flatMap(ally ->
                        translationOpt.map(translation -> ally == Alliance.Red ? flip(translation) : translation)
                    )
                : translationOpt;
    }
}
