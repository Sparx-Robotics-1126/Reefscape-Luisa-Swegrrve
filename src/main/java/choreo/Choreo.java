...
public final class Choreo {
     ...
     */
    public interface TrajectoryLogger<ST extends TrajectorySample<ST>> extends BiConsumer<Trajectory<ST>, Boolean> {}
        ...
        if (files != null) {
            for (File file : files) {
                if (file.getName().endsWith(TRAJECTORY_FILE_EXTENSION)) {
                    trajectories.add(
                        file.getName().substring(0, file.getName().length() - TRAJECTORY_FILE_EXTENSION.length())
                    );
                }
            }
        }
     ...
     */
    static Trajectory<? extends TrajectorySample<?>> loadTrajectoryString(String trajectoryJsonString) {
        ...
        if (sampleType.equals("Swerve")) {
            ...
            return new Trajectory<DifferentialSample>(name, List.of(sampleArray), List.of(splits), List.of(events));
        } else {
