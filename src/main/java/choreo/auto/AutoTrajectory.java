...
public class AutoTrajectory {
    ...
    private void logTrajectory(boolean starting) {
        ...
        if (sample instanceof SwerveSample) {
            ...
            TrajectoryLogger<DifferentialSample> differentialLogger = (TrajectoryLogger<
                    DifferentialSample
                >) trajectoryLogger;
    ...
    public Trigger atPose(Pose2d pose, double toleranceMeters, double toleranceRadians) {
        ...
        return new Trigger(routine.loop(), () -> {
            if (allianceCtx.allianceKnownOrIgnored()) {
                final Pose2d currentPose = poseSupplier.get();
                if (allianceCtx.doFlip()) {
                    boolean transValid =
                        currentPose.getTranslation().getDistance(flippedPose.getTranslation()) < toleranceMeters;
    ...
    public Pose2d[] collectEventPoses(String eventName) {
        ...
        for (int i = 0; i < times.length; i++) {
            Pose2d pose = trajectory.sampleAt(times[i], false).map(TrajectorySample::getPose).get(); // the event times are guaranteed to be valid
            poses[i] = pose;
        }
        return poses;
    }
