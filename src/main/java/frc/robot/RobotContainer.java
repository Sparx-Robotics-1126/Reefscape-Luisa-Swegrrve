...
import frc.robot.commands.subsystems.arm.MoveExtensionToPos;
import frc.robot.commands.subsystems.climb.ClimbMoveToPos;
import frc.robot.commands.subsystems.placer.AcquireCoral;
import frc.robot.commands.subsystems.placer.AnalogPlacer;
...
import frc.robot.subsystems.Swegrrve;
...
public class RobotContainer {
    ...
    public static CommandXboxController m_driver = new CommandXboxController(
        Constants.GeneralConstants.DRIVER_CONTROLLER_ID
    );
    public static CommandXboxController m_operator = new CommandXboxController(
        Constants.GeneralConstants.OPERATOR_CONTROLLER_ID
    );
    ...
    static final SendableChooser<Command> m_chooser = new SendableChooser<>();

    // The robot's subsystems and commands are defined here...
    static final Swegrrve m_swerve = new Swegrrve();
    ...
    //     .copy()
    //     .withControllerHeadingAxis(m_driver::getRightX, m_driver::getRightY)
    //     .headingWhile(true);
    ...
    //         () -> Math.sin(m_driver.getRawAxis(2) * Math.PI) * (Math.PI * 2),
    //         () -> Math.cos(m_driver.getRawAxis(2) * Math.PI) * (Math.PI * 2))
    //     .headingWhile(true)
    ...
    public RobotContainer() {
        ...
        // m_climb.setDefaultCommand(new ClimbMoveArm(()->
        // m_operator.getRawAxis(XboxController.Axis.kLeftX.value), m_climb));
        m_arm.setDefaultCommand(
            new ControllerMoveArm(() -> m_operator.getRawAxis(XboxController.Axis.kLeftY.value), m_arm)
        );
        // m_extension.setDefaultCommand(new ControllerMoveExtension(()->
        // m_operator.getRawAxis(XboxController.Axis.kRightY.value), m_extension));
        m_extension.setDefaultCommand(new MoveExtHome(m_extension, .05));

        ledSubsystem.setDefaultCommand(new TeamLights(ledSubsystem));

        // m_placer.setDefaultCommand(new AnalogPlacer(()->
        // m_operator.getRawAxis(XboxController.Axis.kLeftY.value), m_placer));

        // m_algae.setDefaultCommand(new MoveAlgae(m_algae, () ->
        // m_operator.getRawAxis(XboxController.Axis.kRightY.value)));
    ...
    private void configureDriverBindings() {
        m_swerve.setDefaultCommand(m_swerve.drive(this::driverX, this::driverY, this::driverAngular));
        if (Robot.isSimulation()) {
            ...
            // )
            // );
            // m_driver.start().onTrue(Commands.runOnce(() -> m_swerve.resetOdometry(new
        ...
        if (DriverStation.isTest()) {
            ...
            // m_driver.x().whileTrue(Commands.runOnce(m_swerve::lock,
            // m_swerve).repeatedly());
            ...
            // TODO: should be resetPose()?
            m_driver.leftTrigger().onTrue(m_swerve.tareRotation());
    ...
    public void configureOperatorBindings() {
        m_operator
            .povDown()
            .whileTrue(new MoveArmToAngle(m_arm, 0).alongWith(new MoveExtensionToPos(m_extension, m_arm, 0.01))); // arm home
        m_operator
            .povUp()
            .whileTrue(
                new MoveArmToAngle(m_arm, 18.442849922180176)
                    .alongWith(new MoveExtensionToPos(m_extension, m_arm, .01))
                    .alongWith(new IngestCoral(m_placer, -.5).andThen(new PositionCoral(m_placer)))
            ); // arm to coral
        // station
        ...
        m_operator
            .a()
            .whileTrue(
                new MoveArmToAngle(m_arm, ArmConstants.L1_ARM_POS)
                    .alongWith(new MoveExtensionToPos(m_extension, m_arm, 0.013659))
                    .alongWith(new ReefLights(ledSubsystem, true, 1))
            ); // arm l1
        m_operator
            .x()
            .whileTrue(
                new MoveArmToAngle(m_arm, ArmConstants.L2_ARM_POS)
                    .alongWith(new MoveExtensionToPos(m_extension, m_arm, -0.0831989))
                    .alongWith(new ReefLights(ledSubsystem, true, 2))
            ); // arm l2
        m_operator
            .b()
            .whileTrue(
                new MoveArmToAngle(m_arm, ArmConstants.L3_ARM_POS)
                    .alongWith(new MoveExtensionToPos(m_extension, m_arm, -0.25))
                    .alongWith(new ReefLights(ledSubsystem, true, 3))
            ); // arm l3
        m_operator
            .y()
            .whileTrue(
                new MoveArmToAngle(m_arm, ArmConstants.L4_ARM_POS)
                    .alongWith(new MoveExtensionToPos(m_extension, m_arm, -0.55))
                    .alongWith(new ReefLights(ledSubsystem, true, 4))
            ); // arm l4

        m_operator
            .rightTrigger(0.1)
            .whileTrue(
                new AnalogPlacer(() -> m_operator.getRawAxis(XboxController.Axis.kRightTrigger.value), m_placer, false)
            );
        m_operator
            .leftTrigger(0.1)
            .whileTrue(
                new AnalogPlacer(() -> m_operator.getRawAxis(XboxController.Axis.kLeftTrigger.value), m_placer, true)
            );
    ...
    public void configurePathPlanner() {
        ...
        NamedCommands.registerCommand(
            "MoveExtensionToCoral",
            new MoveExtensionToPos(m_extension, m_arm, .01).withTimeout(1)
        );
        NamedCommands.registerCommand(
            "AcquireCoral",
            new IngestCoral(m_placer, -.35).andThen(new PositionCoral(m_placer))
        );
        ...
        NamedCommands.registerCommand(
            "MoveExtensionToL1",
            new MoveExtensionToPos(m_extension, m_arm, 0.013659).withTimeout(1)
        );
        NamedCommands.registerCommand(
            "MoveExtensionToL2",
            new MoveExtensionToPos(m_extension, m_arm, -0.0831989).withTimeout(1)
        );
        NamedCommands.registerCommand(
            "MoveExtensionToL3",
            new MoveExtensionToPos(m_extension, m_arm, -0.25).withTimeout(1)
        );
        NamedCommands.registerCommand(
            "MoveExtensionToL4",
            new MoveExtensionToPos(m_extension, m_arm, -0.55).withTimeout(2)
        );
        ...
        // NamedCommands.registerCommand("MovetoPos", new InstantCommand(() ->
        // m_swerve.moveToPosition) );
    }
    ...
    public double driverAngular() {
        return m_driver.getLeftTriggerAxis() - m_driver.getRightTriggerAxis();
    }
}
