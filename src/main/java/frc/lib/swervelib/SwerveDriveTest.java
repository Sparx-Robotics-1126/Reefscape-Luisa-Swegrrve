...
public class SwerveDriveTest {
  ...
  public static void setModulesToRotaryPosition(SwerveDrive swerveDrive) {
    SwerveModuleState[] rotaryStates =
        swerveDrive.kinematics.toSwerveModuleStates(new ChassisSpeeds(0, 0, 1));
    for (int i = 0; i < swerveDrive.getModules().length; i++) {
      swerveDrive
          .getModules()[i]
          .getAngleMotor()
          .setReference(rotaryStates[i].angle.getDegrees(), 0);
    }
  }
   ...
   * @param swerveDrive {@link SwerveDrive} to control.
   * @param volts Voltage to send to drive motors.
   * @param testWithSpinning - Whether to make the robot spin in place instead of driving in a
   *     straight line, true to make the robot spin, false to make the robot drive in straight line
   */
  public static void runDriveMotorsCharacterizationOnSimModules(
      SwerveDrive swerveDrive, double volts, boolean testWithSpinning) {
    SwerveModuleState[] rotaryStates =
        swerveDrive.kinematics.toSwerveModuleStates(new ChassisSpeeds(0, 0, 1));
    for (int i = 0; i < swerveDrive.getModules().length; i++) {
      swerveDrive
          ...
          .runDriveMotorCharacterization(
              testWithSpinning ? rotaryStates[i].angle : Rotation2d.kZero, volts);
    }
  }
            ...
            (Voltage voltage) -> {
              if (!SwerveDriveTelemetry.isSimulation) {
                ...
                SwerveDriveTest.runDriveMotorsCharacterizationOnSimModules(
                    swerveDrive, voltage.in(Volts), testWithSpinning);
              }
            },
            log -> {
              for (SwerveModule module : swerveDrive.getModules()) {
